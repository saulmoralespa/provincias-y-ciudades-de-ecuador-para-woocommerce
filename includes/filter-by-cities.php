    ...
    if ( ! class_exists( 'Filters_By_Cities_Method' ) ) {
        ...
        class Filters_By_Cities_Method extends WC_Shipping_Method
        {
            ...
            public function __construct($instance_id = 0)
            {
                ...
                $this->method_title       = __( 'Shipping filter By Cities', 'provincias-y-ciudades-de-ecuador-para-woocommerce' );
                $this->method_description = __( 'Allows adding rules by city', 'provincias-y-ciudades-de-ecuador-para-woocommerce' );
            ...
            /**
             * generate_rules_shipping_html function.
             *
             * @access public
             * @return string
             */
            public function generate_rules_shipping_html($key, $data)
            {
                ob_start();
                include_once 'admin/html/html-rules-shipping.php';
                return ob_get_clean();
            }

            public function validate_rules_shipping_field( $key, $value )
            {
                return $value;
            }
            ...
            public function calculate_shipping( $package = array() )
            {
                ...
                if (!empty($method_rules_settings)){
                    $rules = array_filter($method_rules_settings, function($x) use ($weight){
                        return $x['min'] <= $weight && $weight <= $x['max'];
                    });
                }

                if (!empty($rules)){
                    $rules = current($rules);
                    $rate['cost'] += $rules['additional_cost'];
                }
